...
public class BaseRecipeGroupButtonWidget extends ImageButton {
	...
	private void extractIcons(GuiGraphicsExtractor context) {
		int i = this.groupFocused ? -2 : 0;
		if (this.tab.secondaryIcon().isPresent()) {
			context.fakeItem(this.tab.primaryIcon(), this.getX() + 3 + i, this.getY() + 5);
			context.fakeItem(this.tab.secondaryIcon().get(), this.getX() + 14 + i, this.getY() + 5);
		} else {
			context.fakeItem(this.tab.primaryIcon(), this.getX() + 9 + i, this.getY() + 5);
		}
	}
