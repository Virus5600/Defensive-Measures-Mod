...
public class FlakCloudParticle extends BillboardParticle {
	// CONSTRUCTORS //
	public FlakCloudParticle(ClientWorld level, double x, double y, double z, SpriteProvider spriteSet, double xd, double yd, double zd) {
		...
		this.gravityStrength = 0.06125f;
		this.collidesWithWorld = true;

		this.velocityMultiplier = 0.5f;
		this.setVelocity(xd, yd, zd);

		float black = this.random.nextFloat() * 0.3F;

		this.red = black;
		this.green = black;
		this.blue = black;

		this.scale = 0.1F * (this.random.nextFloat() * this.random.nextFloat() * 15.0F + 10.0F);
	...
	public static class Factory implements ParticleFactory<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType type, ClientWorld level, double x, double y, double z, double xd, double yd, double zd, Random random) {
			return new FlakCloudParticle(level, x, y, z, this.spriteProvider, xd, yd, zd);
		}
	}
}
