 ...
 */
public class BallistaTurretItem extends TurretItem implements FuelItem {
	public BallistaTurretItem(EntityType<? extends Mob> type, Properties settings) {
		super(
			type,
			settings
				.stacksTo(16)
	...
	public void appendHoverText(ItemStack stack, @NonNull TooltipContext context, @NonNull TooltipDisplay displayComponent, @NonNull Consumer<Component> textConsumer, @NonNull TooltipFlag type) {
		...
		for (int i = 1; i <= 3; i++) {
			textConsumer.accept(
				Component.translatable("itemTooltip.dm.ballista.line" + i)
